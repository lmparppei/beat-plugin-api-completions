...
export function activate(context: vscode.ExtensionContext) {
	...
	function BCompletion (str: string, doc = "", type:vscode.CompletionItemKind = vscode.CompletionItemKind.Method, insertText = "") {
		const completion = new vscode.CompletionItem(str, type);

		if (doc.length > 0) completion.documentation = new vscode.MarkdownString(doc);
		if (insertText.length > 0) completion.insertText = insertText;
